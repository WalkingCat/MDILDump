...
// 				printf_s("\tRawDataSize = %d, RawDataAddress = 0x%x\n", mdilSize, mdilOffset);
		...
		if ((mdilOffset > 0) && (mdilSize > 0)) {
			...
			if (!file.read(data.method_map.raw, data.header->methodMapCount)) return g_file_reading_error;
			if (!file.read(data.generic_instances.raw, data.header->genericInstSize)) return g_file_reading_error;
